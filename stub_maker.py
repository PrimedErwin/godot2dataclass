from gdt_to_dataclass import load_extension_api, ClassesSingle, BuiltinClass, GlobalEnum
...
def single_class_to_pyi(single_class: ClassesSingle, pyi_w: Writer) -> None:
    ...
    if single_class.enums:
        for e in single_class.enums:
            pyi_w.write(f'class {single_class.name}_{e.name}:')
            pyi_w.indent()
            enum_description = ''
            if e.description:
...
all_in_one = load_extension_api('extension_api.json')
...
with open('godot_class.pyi', 'w', encoding='utf-8') as fp:
    fp.write(str(pyi_w))
